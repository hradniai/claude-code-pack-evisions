...
**The status line.** If you went through the optional settings step, the bottom of the Claude Code window shows a few short lines: the model Claude is using, your project and git branch (a separate line of work in git), how full the conversation is (the bar of little blocks), how much of your 5-hour and 7-day usage limit you have used and when each resets, and what the session has cost so far. It adjusts to the width of the window, so it also reads on a phone: a narrow screen gets more, shorter lines, with the model, the conversation bar and the usage limits at the top. When the conversation is nearly full, Claude will soon condense its older part, so that is a good moment for `/checkpoint`.
